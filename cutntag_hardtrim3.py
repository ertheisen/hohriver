...
for i in range(len(input_files_R1)):
#create string for system command
	temp_str = 'trim_galore_dev -o /data/hardtrim_out --hardtrim5 60 ' + input_files_R1[i]
...
for i in range(len(input_files_R2)):
#create string for system command
	temp_str = 'trim_galore_dev -o /data/hardtrim_out --hardtrim5 60 ' + input_files_R2[i]
...
if spike_species in ['dm6', 'sacCer3', 'ecoli']:
	...
	totalreads = []
	for i in range(len(spikecount)):
		totalreads.append(spikecount[i]+humancount[i])

	scaling_factor = []
	for i in range(len(spikecount)):
		scaling_factor.append(((float(max(totalreads))/float(totalreads[i]))*(float(genome_size)))/((float(max(totalreads))/float(totalreads[i]))*(float(spikecount[i]))))

	for i in range(len(spikefiles)):
		count = str(spikecount[i])
		hscount = str(humancount[i])
		totcount = str(totalreads[i])
		print('\n')
		print('for ' + spikefiles[i] + ' total / human / spike count:') 
		print(totcount + ' / ' + hscount + ' / ' + count)
		print('\n')
	...
	print('No scaling factor applied')
	print('\n')
	#run bedtools genomecov to generate bedgraph files
	for i in range(len(bg_names)):
		BedTool(bed_names[i]).genome_coverage(bg = True, genome = 'hg19', scale = 1).moveto(bg_names[i])

	for i in range(len(size_selected_small_bg)):
		BedTool(size_selected_small[i]).genome_coverage(bg = True, genome = 'hg19', scale = 1).moveto(size_selected_small_bg[i])

	for i in range(len(size_selected_med_bg)):
		BedTool(size_selected_med[i]).genome_coverage(bg = True, genome = 'hg19', scale = 1).moveto(size_selected_med_bg[i])

	for i in range(len(size_selected_big_bg)):
		BedTool(size_selected_big[i]).genome_coverage(bg = True, genome = 'hg19', scale = 1).moveto(size_selected_big_bg[i])
